...
const IGNORE_TIMES_CHANNELS =
  process.env.IGNORE_TIMES_CHANNELS != undefined
    ? process.env.IGNORE_TIMES_CHANNELS.split(',')
    : [];
...
app.message(
  ...
  async ({ message, say }) => {
    ...
    let rankedChannels = channels
      .filter(
        (c) =>
          c.name.includes('times') && !IGNORE_TIMES_CHANNELS.includes(c.id),
      )
